...
class GeoLocationViewSetTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.utrecht_loc = GeoLocation.objects.create(
            location=self.utrecht, timestamp=make_aware(datetime.now())
        )
    ...
    def test_create_location_with_coordinates(self):
        ...
        data = {"coordinates": [5.2913, 52.1326]}  # Hilversum
    ...
    def test_create_location_with_lat_lng(self):
        ...
        data = {"latitude": 51.8125, "longitude": 5.8372}  # Nijmegen
        response = self.client.post(self.list_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(GeoLocation.objects.count(), 4)
    ...
    def test_create_location_with_timestamp(self):
        ...
        data = {"coordinates": [4.7683, 50.8375], "timestamp": timestamp}  # Brussels
        response = self.client.post(self.list_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_create_location_invalid_coordinates(self):
        ...
        data = {"coordinates": [500, 500]}  # Invalid coordinates
        response = self.client.post(self.list_url, data, format="json")
    ...
    def test_create_location_missing_coordinates(self):
        ...
        data = {"timestamp": "2024-03-15T12:00:00Z"}
        response = self.client.post(self.list_url, data, format="json")
    ...
    def test_create_location_invalid_data_format(self):
        ...
        response = self.client.post(
            self.list_url, "This is not valid JSON", content_type="application/json"
        )
...
class GeoLocationAPIUserTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.location3 = GeoLocation.objects.create(
            location=point3, user=self.user2, timestamp=make_aware(datetime.now())
        )
    ...
    def test_filter_by_user(self):
        ...
        self.assertEqual(
            total_locations, 3, "Setup should create exactly 3 GeoLocation objects"
        )
        ...
        user1_locations = [
            loc
            for loc in results
            if loc.get("user") and loc["user"]["id"] == self.user1.id
        ]
        self.assertEqual(
            len(user1_locations),
            2,
            f"User1 should have 2 locations, found {len(user1_locations)}",
        )
        ...
        user2_locations = [
            loc
            for loc in results
            if loc.get("user") and loc["user"]["id"] == self.user2.id
        ]
        self.assertEqual(
            len(user2_locations),
            1,
            f"User2 should have 1 location, found {len(user2_locations)}",
        )
